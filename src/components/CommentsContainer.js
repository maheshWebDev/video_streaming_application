import React from "react";

const commentsData = [
  {
    name: "User123",
    text: "This video is amazing! I can't believe how informative it is. 🌟",
    replies: [],
  },
  {
    name: "CoolViewer456",
    text: "The content is top-notch. Kudos to the creator! 👏",
    replies: [
      {
        name: "SuperFan789",
        text: "Absolutely agree! I've learned so much from this channel. 🧠",
        replies: [
          {
            name: "CuriousMind101",
            text: "I watch every video religiously. Keep up the good work! 🎉",
            replies: [
              {
                name: "VideoEnthusiast555",
                text: "Your videos are a game-changer for me. Thanks a lot! 🚀",
                replies: [
                  {
                    name: "SupportiveViewer222",
                    text: "I appreciate the effort you put into each video. Thumbs up! 👍",
                    replies: [],
                  },
                ],
              },
            ],
          },
        ],
      },
    ],
  },
  {
    name: "NewSubscriber777",
    text: "Just subscribed! This channel is a hidden gem. 💎",
    replies: [
      {
        name: "ExcitedWatcher888",
        text: "Welcome to the community! You're in for a treat. 🎈",
        replies: [
          {
            name: "ReturningFan333",
            text: "I've been following for months. Never disappointed! 😊",
            replies: [
              {
                name: "FrequentCommenter444",
                text: "Can't stop watching your videos. Addicted! 📺",
                replies: [],
              },
              {
                name: "YouTubeExplorer999",
                text: "Discovered this channel recently. Instantly hooked! 🔍",
                replies: [
                  {
                    name: "InquisitiveMind777",
                    text: "Your content is a breath of fresh air. Loving it! 🌿",
                    replies: [],
                  },
                ],
              },
              {
                name: "CasualViewer111",
                text: "Just stumbled upon this. Pleasantly surprised! 😄",
                replies: [],
              },
            ],
          },
        ],
      },
    ],
  },
];

// console.log(commentsData);

const Comment = ({ data }) => {
  const { name, text, replies } = data;
  return (
    <div className="flex shadow-sm bg-gray-100 p-2 rounded-lg my-1">
      <img
        className="w-12 h-12"
        src="data:image/png;base64,"
        alt="user"
      />
      <div className="px-3">
        <p className="font-bold">{name}</p>
        <p>{text}</p>
      </div>
    </div>
  );
};

const CommentsList = ({ comments }) => {
  return comments.map((comment, index) => (
    <div key={index}>
      <Comment data={comment} />
      <div className="pl-5 border border-l-black ml-5">
        <CommentsList comments={comment.replies} />
      </div>
    </div>
  ));
};

const CommentsContainer = () => {
  return (
    <div className="m-5 p-2">
      <h1 className="text-2xl font-bold">Comments :</h1>
      <CommentsList comments={commentsData} />
    </div>
  );
};

export default CommentsContainer;
